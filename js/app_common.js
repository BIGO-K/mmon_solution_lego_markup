...
mm.is = (function () {
	...
	(function () {
		...
		// 모바일
		if (base.isMobile()) {
			classes.push('S=mobile');

			if (base.isMobile('ios')) classes.push('S=ios');
			if (base.isMobile('app')) {
				classes.push('S=app');// 앱으로 접속 (앱에 userAgent 코드 추가 필요)

				if (base.isMobile('app_kitkat')) classes.push('S=kitkat');// 안드로이드 4.4.x 킷캣버전
			}
		}
		// pc
		else {
			...
			// IE
			if (base.isIE()) {
				if (base.isIE('ie6')) classes.push('S=ie6');
				else if (base.isIE('ie7')) classes.push('S=ie7');
				else if (base.isIE('ie8')) classes.push('S=ie8');
				else if (base.isIE('ie9')) classes.push('S=ie9');
				else if (base.isIE('ie10')) classes.push('S=ie10');
				else if (base.isIE('ie11')) classes.push('S=ie11');
				else if (base.isIE('edge')) classes.push('S=edge');
			}

			// whale
			if (/whale/i.test(navigator.userAgent)) classes.push('S=whale');
		}
	...
	return {
		...
		layout: function (__type) {
			...
			return document.documentElement.classList.contains(mm.string.template('L=${TYPE}', { TYPE: __type }));
			...
			default:
				if (_trim.startsWith('data-')) return $parent.querySelectorAll(mm.selector(_trim, '[]'));// data attribute
					...
					if (splitTotal === 1) return $parent.getElementsByTagName(_trim);
						...
						return $parent.querySelectorAll(mm.string.template(':scope ${SELECTOR}', { SELECTOR: _trim.replace(/\,/g, ', :scope ').replace(/S\=/g, 'S\\=') }));
					}
				}
		}
...
mm.scroll = (function () {
	...
	// UI 고유 객체
	var base = {
		_classNo: 'S=noscroll',// ? :string - 스크롤 차단 클래스
		// 스크롤 검색
		...
		toggle: function (__is) {
			...
			if (mm.class.some($html, ['L=bom', 'L=modal'])) return;// BOM 이나 모달이 있으면 false로 유지
...
mm.element = (function () {
	...
	var base = {
		...
		// 단일 요소에 DOM 추가
		insertNode: function (__$element, __appends, __local, __index) {

			// ? __$element:element - 단일 요소
			// ? __appends:string = html 코드
			// ? __appends:element
			// ? __local:string - before, after, prepend, append
			// ? __index:number - __$element의 인덱스로 0보다 크면 다중으로 clone 추가
			...
			switch (mm.string.constructor(__appends)) {
				...
				case 'NodeList':
					...
					break;
				case 'Array':
					_.forEach(__appends, function (__append) {
			...
			_.forEach($appends, function (__$append) {

				var $append = (!__index || __index === 0) ? __$append : __$append.cloneNode(true);
				switch (__local) {
					case 'after':
						__$element.parentElement.insertBefore($append, __$element.nextSibling);
						break;
					case 'before':
						__$element.parentElement.insertBefore($append, __$element);
						break;
					case 'append':
						__$element.append($append);
						break;
					case 'prepend':
						__$element.prepend($append);
						break;
				}
		...
		toggleDisplay: function (__$element, __is) {
			...
			else base.setStyle(__$element, { 'display': 'none' });
		...
		findIndex: function (__$lists, __element) {
			...
			// ? __element:element/string - 단일 요소 또는 셀렉터

			if (__$lists.length === 0 || !__element) return -1;

			if (mm.is.element(__element)) return __$lists.indexOf(__element);
			else if (typeof(__element) === 'string') {
				if (__element.includes('=')) __element = __element.replace(/\=/g, '\\=');
				return __$lists.indexOf(_.find(__$lists, function (__$item) { return __$item.matches(__element); }));
			}
	...
	return {
		...
		attribute: function (__elements, __attribute) {
			...
			_.forEach(mm.find(__elements), function (__$el) {
		...
		//- 텍스트
		text: function (__elements, __text) {

			// ? __elements:element
			// ? __text:string - 요소에 적용할 문자열

			if (__text.length === 0) return;

			_.forEach(mm.find(__elements), function (__$el) {

				__$el.textContent = __text;
		...
		after: function (__elements, __appends) {
			...
			_.forEach($elements, function (__$el, __index) {
				...
				base.insertNode(__$el, __appends, 'after', __index);
		...
		before: function (__elements, __appends) {
			...
			_.forEach($elements, function (__$el, __index) {
				...
				base.insertNode(__$el, __appends, 'before', __index);
		...
		append: function (__elements, __appends) {
			...
			_.forEach($elements, function (__$el, __index) {
				...
				base.insertNode(__$el, __appends, 'append', __index);
		...
		prepend: function (__elements, __appends) {
			...
			_.forEach($elements, function (__$el, __index) {
				...
				base.insertNode(__$el, __appends, 'prepend', __index);
...
mm.changeSyncer = function (__elements, __is, __dataName) {
	...
	var _class = mm.string.template('S=${NAME}-use', { NAME: __dataName.replace('data-', '') });
	var $elements = mm.find(__elements);
				...
				if (_isIframe) {
					...
					*/
					...
					if (data._isRatio === true) {
						...
						var _classRatio = 'S=image-square';
...
mm.preload = (function () {
	...
	var base = {
		...
		get _classLoading() { return 'S=loading'; },
		get _classLoaded() { return 'S=loaded'; },
		get _classError() { return 'S=error'; },
	};
...
mm.lazyload = (function () {
	...
	var initial = {
		...
		_rootMargin: '50% 0px',// ? :string - intersection observer 영역의 rootMargin
		onBefore: null,// ? :function
		onBeforeParams: [],// ? :array
		onComplete: null,// ? :function
		onCompleteParams: [],// ? :array
...
mm.tab = (function () {
	...
	return {
		...
		change: function (__elements, __target) {
			...
			// __target 값이 없으면 mm_tab에서 S=on 클래스가 있는 요소의 index에 적용
...
mm.carousel = (function () {
	...
	var base = {
		...
		update: function (__$element) {
			...
			__$element.classList.add('S=ready');// update가 적용될 때 figcaption 노출
...
mm.slider = (function () {
	...
	var initial = {
		...
		_speed: 0.2,// ? :number - 움직임 시간(초)
		_sensitiveTime: 0.2,// ? :number - 짧게 움직였을 때 반응할 시간(초)
		_rootMargin: '10px 1px',// ? :string - intersection observer 영역의 rootMargin
		_isLoop: true,// ? : boolean - 슬라이더 반복
		_isErrorRemove: false,// ? :boolean - 프리로드 요소가 오류일 때 mm_slider-item 요소 삭제
		_classIntersecting: 'S=intersecting',// ? :string - 영역 내 intersecting 된 전체 요소
		_classOn: 'S=on',// ? :string - 영역 내 온전히(ratio 1) 노출된 전체 요소
		_classClone: 'S=clone',// ? :string
		pagination: {
			_isInner: true,// ? :boolean - 페이지네이션 버튼을 ui 안에서 찾기
			_el: '.btn_slider-page',
				...
				mm.event.on(document, 'mousemove mouseup', function sliderTouchInlineHandler(__e) {
					...
					if (data.__._isDirection === null) {
						...
						if (__e.type === 'mousemove') base.gsapUpdate(__$element, _direction);
							...
							var _touchTime = mm.time.stampEnd('slider') / 1000;// 빠른 드래그 확인
							var _index = null;
		...
		gsapTo: function (__$element, __direction, __ease, __index) {
			...
			gsap.to(data.__, {
				...
				onComplete: function () {
					...
					// base.set(__$element);// intersection과 중복 테스트 필요
...
mm.form = (function () {
	...
	var base = {
		...
		liftFormText: function (__$element) {
			...
			while (_i < $ui.classList.length) {
				...
				if (_class.startsWith('S=text-alert') || _class.startsWith('S=text-valid')) $ui.classList.remove(_class);
				else _i++;
			}
	...
	(function () {
		...
		mm.delegate.on(document, mm.selector(['data-text', 'data-select'], '[]'), 'focusin focusout', function (__e) {
			...
			switch (__e.type) {
				case 'focusin':
					if (this.hasAttribute('data-text')) document.documentElement.classList.add('S=focus');
					...
					break;
				case 'focusout':
					document.documentElement.classList.remove('S=focus');
					break;
			}
	...
	return {
		...
		valid: function (__elements, __message, __condition) {
			...
			_.forEach($elements, function (__$el) {
				...
				var $text = base.appendFormText(__$el, mm.string.template('<p class="${CLASS}">${MESSAGE}</p>', { CLASS: base._classValid, MESSAGE: _message }));
				...
				$text.classList.add(mm.string.template('S=valid-${CLASS}', { CLASS: _condition }));
				$ui.classList.add(mm.string.template('S=text-valid-${CONDITION}', { CONDITION: _condition }));
...
mm.form.text = (function () {
	...
	var initial = {
		...
		_classOn: 'S=on',// ? :string - value 값이 있을 때 클래스
		_classOff: 'S=text-off',// ? :string - value 값이 있고, readonly, disabled 상태일 때 클래스
		...
		_byte: 0,// ? :number - 0보다 크면 자동 생성
		_isAutoComplete: false,// ? :boolean - value값 입력시 연관검색어 자동완성
		// 내부사용
		__: {
			$byte: null,// ? :element - 바이트 객체(바이트 적용 여부 확인으로 사용)
	...
	var base = {
		...
		byte: (__$text) => {
			...
			var $ui = __$text.closest('.mm_form-textarea');// .mm_form-text 일단 제외
			var data = mm.data.get(__$text, base._dataName);
			if (!$ui || data.__.$byte) return;

			const _classByte = `S=byte`;
			let $byte = data.__.$byte = mm.find(`.text_byte`, $ui)[0];
			if ($byte == null) {
				$byte = data.__.$byte = mm.element.create(`<b class="text_byte"><strong>0</strong>/${data._byte}</b>`)[0];
				mm.element.append($ui, $byte);
			}

			$ui.classList.add(_classByte);

			mm.event.on(__$text, `keydown keyup change`, function byteKeydownHandler(__e) {

				let _value = __$text.value;
				if (_value.length > data._byte) _value = __$text.value = _value.substring(0, data._byte);
				$byte.firstElementChild.textContent = _value.length;

				// if (__$text.tagName === `INPUT`) {
				// 	__$text.style.width = ``;
				// 	__$text.style.width = `${__$text.scrollWidth}px`;
				// }
		...
		autocomplete: function (__$text) {
			...
			function keyDownFocus(__e, __$el) {
				...
				mm.delay.on(function () {
					...
					mm.class.remove(mm.find('.S=over', data.__.$autocomplete), 'S=over');
			...
			mm.event.on($ui, 'keydown mouseover mouseenter mouseleave focusin focusout', function (__e) {
				...
				switch (__e.type) {
					case 'keydown':
						var $active = mm.find('.S=over', data.__.$autocomplete)[0] || document.activeElement;
					...
					case 'mouseover':// 리스트 아이템에 hover시 포커스
						...
						if ($autoItem) $autoItem.classList.add('S=over');
						break;
					// case 'mouseenter':
					// case 'focusin':
					// 	break;
	...
	(function () {
		...
		mm.delegate.on(document, mm.selector(base._dataName, '[]'), 'update change paste keydown keyup focusout', function (__e) {
			...
			switch (__e.type) {
				...
				case 'update':
					...
					if (data._byte > 0) base.byte($text);
				...
				case 'keyup':
					...
					if (data._default.trim().length > 0) {
						if (data._default !== _value) $ui.classList.add('S=text-changed');
						else $ui.classList.remove('S=text-changed');
					}
		...
		mm.delegate.on(document, '.btn_text-pw', 'click', function (__e) {
			...
			var $ico = mm.find('.ico_pw-show, .ico_pw-hide', this)[0];
			if ($ico) {
				mm.class.toggle($ico, ['ico_pw-show', 'ico_pw-hide']);
				$ico.nextElementSibling.textContent = ($ico.classList.contains('ico_pw-show')) ? '비밀번호 숨기기' : '비밀번호 보기';
			}
...
mm.form.file = (function () {
	...
	var base = {
		...
		get _classFull() { return 'S=image-full'; },
		get _classFit() { return 'S=image-fit'; },
		// initial type
		...
		appendImage: function (__$file, __$canvas) {
			...
			if (Number.isFinite(data._imageRatio)) {
				...
				$ui.classList.add('S=image-ratio');
...
mm.form.multiple = (function () {
	...
	var base = {
		...
		checkButton: function (__$multiple) {
			...
			if (mm.find('.S=image-on', $ui).length > 1) mm.element.show(mm.find('.btn_remove-all, .mm_form-multiple-sortable', $ui));
	...
	(function () {
		...
		mm.delegate.on(document, '.mm_form-multiple [class*=btn_sort]', 'click', function (__e) {
			...
			if (this.classList.contains('btn_sort')) {
				$ui.classList.add('S=multiple-sortable');
				mm.element.append(mm.find('.mm_form-image', $fileList), '<i class="ico_image-sortable"></i>');

				if (!data._isMax && data._isAuto) {
					var $reject = mm.closest(_.filter(mm.find('.mm_form-image', $ui), function (__$item) { return !__$item.classList.contains('S=image-on'); }), 'li');
				...
				$ui.classList.remove('S=multiple-sortable');
...
mm.bom = (function () {
	...
	var base = {
		...
		resizeBom: function (__$bomItem) {
			...
			var $container = (__$bomItem) ? mm.find('> .mm_bom-item-inner', __$bomItem)[0] : mm.find('.mm_bom-item.S=on > .mm_bom-item-inner', base.$bom)[0];
		...
		appendItem: function (__text, __title, __buttons) {
			...
			document.documentElement.classList.add('L=bom');
			...
			if ($bomItemText.childElementCount === 0) $bomItemText.remove();
		...
		closeItem: function (__callback, __params, __isClose) {
			...
			if (__isClose !== false) {
				...
				// BOM영역 삭제 step.1
					...
					document.documentElement.classList.remove('L=bom');
				}
			}
	...
	return {
		...
		alert: function (__text, __callback, __option) {
			...
			// * 제목과 내용의 줄바꿈은 \n 또는 <br>로 적용
			// mm.bom.alert('내용');
			// mm.bom.alert('내용', 콜백함수, { _title: '제목', buttons: ['확인'], params: [콜백 파라미터] });
		...
		confirm: function (__text, __callback, __option) {
			...
			// mm.bom.confirm('내용');
			// mm.bom.confirm('내용', 콜백함수, { _title: '제목', buttons: ['확인', '취소'], params: [콜백 파라미터] });
		...
		prompt: function (__text, __callback, __option) {
			...
			// mm.bom.prompt('내용');
			// mm.bom.prompt('내용', 콜백함수, { _title: '제목', buttons: ['확인', '취소'], params: [콜백 파라미터], forms: [{ _type: 'tel', _placeholder: '휴대폰 번호' }, { _type: 'select', _value: 200, attribute: { 'style': 'border-width: 2px;' }, options: [{ _text: '옵션1', _value: 100 }, { _text: '옵션2', _value: 200 }] }] });
...
mm.modal = (function () {
	...
	var base = {
		$modal: null,
		get _classOn() { return 'S=on'; },
		get _classOld() { return 'S=old'; },// 이전 모달
		//- 오프너(window)
		get opener() {
	...
	return {
		...
		open: function (__url, __option) {
			...
			if (option._isFull) $modalItem.classList.add('S=modal-full');
...
mm.colorpicker = (function () {
	...
	var base = {
		...
		appendPicker: function () {
			...
			var colors = [
				...
				['#ffffff', '#f6f6f6', '#d6d6d6', '#b4b4b4', '#a7a7a7', '#828282', '#5e5e5e', '#454545', '#2c2c2c', '#000000']
			];
			...
			base.$picker = mm.element.create(mm.string.template([
				...
				'			<div class="mm_inline">',
				'				<button type="button" class="mm_btn T=line btn_color-cancel"><b>취소</b></button>',
			...
			for (var _i = 0; _i < 10; _i++) {
				_.forEach(colors, function (__items) {
					...
					if (_i > 3) mm.find('.btn_color-chip', $item)[0].classList.add('S=check-white');
			...
			// 	if (__index > 5) mm.find('.btn_color-chip', $item)[0].classList.add('S=check-white');
	...
	return {
		...
		open: function (__$element) {
			...
			// 위치 보정
			var _marginTop = mm.element.offset(__$element).top - mm.element.offset(base.$picker).top;
...
mm.datepicker = (function () {
	...
	var base = {
		...
		create: function (__data, __date, __$datepicker) {
			...
			var $datepicker = (__$datepicker) ? __$datepicker : mm.element.create('<div class="mm_datepicker"></div>')[0];

			// datepicker html구조 생성
			if (!__data.__._isOpen) {
				if (__data.config._isInline) $datepicker.classList.add('S=datepicker-inline');
				else {
				...
				for (var _i = 1; _i <= _itemLength; _i++) {
					var $template = mm.element.create(mm.string.template([
						'<div class="mm_datepicker-item">',
						'	<div class="mm_datepicker-item-head">',
						'		<div class="mm_datepicker...head-label">',
						'			<span></span>',
					...
					if (!__data.config._isMonth) {
						var $headLabel = mm.find('.mm_datepicker\\.\\.\\.head-label', $template)[0];
				...
				var $pickerFoot = mm.element.create(mm.string.template([
					'<div class="mm_datepicker-foot">',
					'	<button type="button" class="mm_btn T=xs T=line T=lighter btn_today" data-datepicker-date="${TODAY}"><b>오늘로 설정</b></button>',
					'</div>',
				], { TODAY: _today }))[0];
			...
			_.forEach(mm.find('.mm_datepicker-item', $datepicker), function (__$item, __index) {
				...
				_.forEach(mm.find('.mm_datepicker\\.\\.\\.head-label', __$item), function (__$label, __index) {
				...
				_.forEach(dates, function (__date, __index) {
					...
					if (!__data.config._isMonth && dateItem.getMonth() + 1 !== _month) $cell.classList.add('S=datepicker-outside-month');

					if (__data.__._periodStart || __data.__._periodEnd) {
						$btnDate.disabled = (function () {

							return (__data.__._periodIndex === 1 && __data.__._periodStart > _dateString) || (__data.__._periodIndex === 0 && __data.__._periodEnd < _dateString);

						})();

						if (__data.__._periodStart === _dateString || __data.__._periodEnd === _dateString) $cell.classList.add('S=datepicker-selected');
						else if (__data.__._periodStart < _dateString && __data.__._periodEnd > _dateString ) $cell.classList.add('S=datepicker-period');
					}
					else if (_dateString === __data.__._pickDate) $cell.classList.add('S=datepicker-selected');
		...
		open: function (__$element) {
			...
			// 기간
			var $period = __$element.closest('.mm_form_mix-period');
			if ($period) {
		...
		close: function () {
			...
			// mm_datepicker 요소는 하나만 생성
			// mm_datepicker가 생성된 상태에서 다른 datepicker를 클릭하면 전체 datepicker의 _isOpen 초기화
			...
			mm.element.remove(mm.find('.mm_datepicker:not(.S=datepicker-inline)'));
		...
		update: function (__elements) {
			...
			_.forEach($elements, function (__$el) {
				...
				data.__.onSelect = function (__e) {
					...
					var $datepicker = $item.closest('.mm_datepicker');
					...
					mm.class.remove(mm.find('.S\\=datepicker-selected', $datepicker), 'S=datepicker-selected');
					mm.class.add(mm.closest(mm.find(mm.string.template('[data-datepicker-date="${VALUE}"]', { VALUE: _pickDate }), $datepicker), 'td'), 'S=datepicker-selected');
				...
				data.__.onDraw = function (__e) {
					...
					var $this = __e.target;
					var $datepicker = $this.closest('.mm_datepicker');
					var $datepickerHeads = mm.find('.mm_datepicker-item-head', $datepicker);
					var changeDate = null;

					switch (__e.type) {
						// 셀렉트
						case 'change':
							var $calendarHead = $this.closest('.mm_datepicker-item-head');
				...
				if (data.config._isInline && !data.__._isOpen) {
					...
					$datepicker.classList.add('S=datepicker-inline');
